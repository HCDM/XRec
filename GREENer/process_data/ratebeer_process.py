...
only_item_attr_graph = True         # attribute nodes only come from item-side
...
class Vocab():
    ...
    def f_load_sent_content_eval(self, sent_content_file):
        # Load sentence content (on valid and test set)
        sent_content = readJson(sent_content_file)[0]
        # Get sentid (on valid and test set)
        sentid_list = list(sent_content.keys())
        # Get number of sentences on train set
        train_sent_num = len(self.m_sent2sid)
        self.m_train_sent_num = train_sent_num
        print("train_sent_num", train_sent_num)
        # Get number of sentences on valid and test set
        sent_num = len(sent_content)
        for sent_idx in range(sent_num):
            # Get sentid_i and sentwords_i
            sentid_i = sentid_list[sent_idx]
            sentwords_i = sent_content[sentid_i]
            # Add number of sentences on train to form an overall sentid_i
            sentid_i = train_sent_num+int(sentid_i)
            sentid_i = str(sentid_i)
            # Map sentid_i with sid_i
            if sentid_i not in self.m_sent2sid:
                sid_i = len(self.m_sent2sid)
                self.m_sent2sid[sentid_i] = sid_i
            # Get sid_i
            sid_i = self.m_sent2sid[sentid_i]
            # Map sid_i with sentwords_i
            self.m_sid2swords[sid_i] = sentwords_i
    ...
    def f_load_sent_embed(self, sent_embed_file):
        """ Load sentid2sentembed and construct sid2sembed
        """
        ### sid 2 embed
...
class RATEBEER(Dataset):
    ...
    def __len__(self):
        """ Get the number of saved graphs """
        ...
        # Number of graphs is saved in graph_summary.txt
        ...
        if os.path.isfile(graph_summary_file):
            # If graph_summary.txt exists, directly read the file_num from this file
            with open(graph_summary_file, "r") as f:
                line_val = f.readline()
                file_num = line_val.strip()
                file_num = int(file_num)
        else:
            # Otherwise, count the number of files (each file is a saved graph)
    ...
    def add_feature_node_item_attr(self, uid, iid, sid_list):
        fid2nid = {}
        nid2fid = {}
        nid = 0
        # Get item-side fids
        itemside_fids = set(self.m_iid2fid2tfidf_dict[iid].keys())
        # Loop through all sids
        for sid in sid_list:
            # Get this sid's connected fid
            fid2tfidf_dict_sent = self.m_sid2fid2tfidf_dict[sid]
            ...
            assert flag
        # fid_node_num = len(nid2fid)
        assert len(nid2fid) == len(fid2nid)
        ### we need to use fid to access feature embedding
    ...
    def create_graph(self, uid, iid, sid_list, label_sid_list):
        ...
        for i in range(sent_node_num):
            ...
            assert sid_i not in sid2nid
            sid2nid[sid_i] = nid_i
            nid2sid[nid_i] = sid_i

        # Number of feature nodes and sentence nodes
        ...
        ### add edges between sents and features
        for i in range(sent_node_num):
            sid = sid_list[i]
            nid_s = sid2nid[sid]
            fid2tfidf_dict_sent = self.m_sid2fid2tfidf_dict[sid]
        ...
        for sid in label_sid_list:
            fid2tfidf_dict_sent = self.m_sid2fid2tfidf_dict[sid]
            # get all features for each gt sentence and label them as 1
            for fid in fid2tfidf_dict_sent:
                nid_f = fid2nid[fid]
...
class RATEBEER_TRAIN(RATEBEER):
    ...
    def create_graph(self, uid, iid, sid_list, label_sid_list):
        ### add feature nodes
        if only_item_attr_graph:
            fid2nid, nid2fid = self.add_feature_node_item_attr(uid, iid, sid_list)
        else:
            fid2nid, nid2fid = self.add_feature_node(uid, iid, sid_list)
        feature_node_num = len(fid2nid)
        ...
        for i in range(sent_node_num):
            ...
            for fid in fid2tfidf_dict_sent:
                if fid in fid2nid:
                    nid_f = fid2nid[fid]
                    tfidf_sent = fid2tfidf_dict_sent[fid]       # tf-idf edge weight
        ...
        ### add edges from user to features
        for i in range(user_node_num):
            nid_u = list(nid2uid.keys())[0]
            fid2tfidf_dict_user = self.m_uid2fid2tfidf_dict[uid]
        ...
        ### add edges from item to features
        for i in range(item_node_num):
            nid_i = list(nid2iid.keys())[0]
            fid2tfidf_dict_item = self.m_iid2fid2tfidf_dict[iid]
...
class RATEBEER_VALID(RATEBEER):
    ...
    def load_raw_data(
        self, vocab, uid2fid2tfidf_dict, iid2fid2tfidf_dict, sid2fid2tfidf_dict,
            sent_content_file, item_feature_file, useritem_candidate_label_sent_file,
            output_graph_dir, save_graph_flag=False):
